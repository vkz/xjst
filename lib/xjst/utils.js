...
function identify(ast) {
  ...
  var keys = visitorKeys[t];    // estraverse.visitorKeys has keys for every possible Esprima ast.type { ast.type : [ prop, ...] }
  if (keys.length)
    out += '->';
  ...
  // examples of out { String }:
  // ------------------------------------------------------------------
  // out:  '{Identifier:__$ctx}'
  // out:  '{Identifier:x}'
  // out:  '{MemberExpression:false->{Identifier:__$ctx},{Identifier:x}}'
  // out:  '{Literal:1}'
