...
function Body(compiler, body, id, rolledOut) {
  ...
  if (body instanceof Body) { // why would it ever be instance of Body?
    // Clone
    var parent = body;
...
// ... where recursive craziness ensue
Body.prototype.rollOutApply = function rollOutApply(ast, type, changes) {
  ...
  if (type !== 'apply') {
    // applyNext, heh
    // this.applyNext accumulates additions to ast we need for applyNext to work out properly between recursive calls to rollOutApply
    if (!this.applyNext) {
      this.applyNext = this.compiler.getApplyNext(), // obtain ast for a unique applyNext flag (actually a hash-prop suitable for 'local(hash)()')
      this.applyNext.member = {                      // ctx.$a5 
    ...
    // add the flag to ctx and apply
    return this.rollOutApply(ast, 'apply', changes.concat( // will be rolled out as local(changes)(f() { apply() }) on reentrance into rollOutApply
...
Body.prototype.rollOutLocal = function rollOutLocal(ast, changes, body) {
  ...
  if (typeof body === 'function') {
    body = body.call(this);     // ??? isn't body an ast, how come we call it
  }
...
//     variable: { type: 'Identifier', name: '__$l2' } } ]
  ...
  // wrap the new unrolled-body in a 'function __$lb() { unrolled-body; return _$r;}'
  // so each local would generate a new function to be experssly called from if-else predicate-tree
  var func = {
