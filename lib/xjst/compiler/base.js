...
// NOTE Prototype for xjst Compiler that implements
// preTranslate: templates -> Templates
// beforeSortGroup: methods sequentially applied to Templates before sortGroup
var proto = {};

proto.preTranslate = function preTranslate(input) {
  // input: Mozilla AST
...
proto.getTemplates = function getTemplates(ast) {
  var other = [],
      init = [];
...
proto.sanitize = function sanitize(stmt) {
  return this.replaceThis(stmt);
};

// Replace `this` with `__$ctx`
proto.replaceThis = function replaceThis(stmt) {
  var ctx = this.ctx;
...
proto.replaceFetch = function replaceFetch(stmt) {
  var self = this;
  return estraverse.replace(stmt, {
    enter: function(node, parent, notify) {
...
proto.checkRef = function checkRef(expr) {
  var self = this;
...
proto.transformTemplates = function transformTemplates(template) {
...
proto.fetchGlobal = function fetchGlobal(name) {
...
proto.registerExtension = function registerExtension(name) {
  if (name !== '__proto__')
    this.extensions[name] = true;
};

proto.getRecordExtensions = function getRecordExtensions() {
...
proto.getResetApplyNext = function getResetApplyNext() {
  var ctx = { type: 'Identifier', name: 'ctx' };
  var body = [];
...
proto.beforeSortGroup = function beforeSortGroup(program) {
  [
    rollOut
...
proto.generate = function generate(code) {
...
proto.compile = function compile(code) {
